package com.classpark.small.coupon;
 ...
 * spring.application.name=small-coupon
 * spring.cloud.nacos.config.server-addr=10.147.19.99:30044
 * 3)给配置中心添加一个默认叫 数据集（Data Id） small-coupon.properties的默认规则，应用名.propertites
 * 4）给 应用名.propertites 添加任何配置
 * 5）动态获取配置
 * @RefreshScope：动态获取并刷新配置
 * @Value("${配置项的名}")：获取到配置 如果配置中心和当前应用的配置文件都配置了相同的项，优先使用配置中心的配置
 * 2.细节
 * 1）命名空间：配置隔离：
 ...
 * 2）配置集：所有配置的集合
 * 3）配置集ID：类似配置文件名
 * Data ID：文件名，例：small-coupon.properties
 * 4）配置分组
